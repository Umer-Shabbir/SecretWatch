...
export function EditFilterDialog({
  ...
  filter: RepositoryFilterSummary;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}) {
  const [type, setType] = useState<RepositoryFilterType>(filter.type);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to update filter rule");
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-lg bg-canvas-default p-6 shadow-xl border border-border-default">
        <h3 className="text-lg font-medium text-fg-default mb-4">Edit Repository Filter Rule</h3>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="edit-enabled" className="text-sm font-medium text-fg-default cursor-pointer">
              Rule enabled
            </label>
          </div>
