...
beforeEach(() => {
  ...
  resetFakeDb([makeRow({ id: "r1", enabled: false })]);
});

describe("POST /api/scan-rules/:id/enable — authorization", () => {
  it("returns 401 when there is no session", async () => {
    currentSessionUserId = null;
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/scan-rules/r1/enable", { method: "POST" }), {
      params: { id: "r1" },
    });
...
describe("POST /api/scan-rules/:id/enable — not found", () => {
  it("returns 404 for a nonexistent id", async () => {
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/scan-rules/nope/enable", { method: "POST" }), {
      params: { id: "nope" },
    });
...
describe("POST /api/scan-rules/:id/enable — success", () => {
  ...
  it("is idempotent when already enabled", async () => {
    resetFakeDb([makeRow({ id: "r2", enabled: true })]);
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/scan-rules/r2/enable", { method: "POST" }), {
      params: { id: "r2" },
    });
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("records an audit log entry", async () => {
    const { POST } = await import("./route");
    await POST(new Request("http://localhost/api/scan-rules/r1/enable", { method: "POST" }), {
      params: { id: "r1" },
    });
    expect(recordAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ userId: "admin-a", action: "scan_rule_enabled" })
    );
  });
});
