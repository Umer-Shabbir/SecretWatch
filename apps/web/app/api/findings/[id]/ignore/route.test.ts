...
describe("POST /api/findings/:id/ignore — authorization", () => {
  it("returns 401 when there is no session", async () => {
    currentSessionUserId = null;
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/findings/f1/ignore", { method: "POST" }), {
      params: { id: "f1" },
    });
...
describe("POST /api/findings/:id/ignore — not found", () => {
  it("returns 404 for a nonexistent id", async () => {
    const { POST } = await import("./route");
    const res = await POST(
      new Request("http://localhost/api/findings/nope/ignore", { method: "POST" }),
      { params: { id: "nope" } }
    );
...
describe("POST /api/findings/:id/ignore — valid transition", () => {
  it("transitions PENDING -> IGNORED and returns the updated finding", async () => {
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/findings/f1/ignore", { method: "POST" }), {
      params: { id: "f1" },
    });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.finding.status).toBe("IGNORED");
    expect(rows.find((r) => r.id === "f1")?.status).toBe("IGNORED");
  });

  it("records an audit log entry with who/when and no raw secret content", async () => {
    const { POST } = await import("./route");
    await POST(new Request("http://localhost/api/findings/f1/ignore", { method: "POST" }), {
      params: { id: "f1" },
    });
    expect(recordAuditMock).toHaveBeenCalledWith(
      expect.objectContaining({ userId: "admin-a", action: "finding_ignored" })
    );
...
describe("POST /api/findings/:id/ignore — invalid transitions", () => {
  it("returns 409 when the finding is not PENDING (e.g. already FLAGGED)", async () => {
    resetFakeDb([makeRow({ id: "f2", status: "FLAGGED" })]);
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/findings/f2/ignore", { method: "POST" }), {
      params: { id: "f2" },
    });
  ...
  it("returns 409 (not a silent success) when re-ignoring an already-IGNORED finding", async () => {
    resetFakeDb([makeRow({ id: "f3", status: "IGNORED" })]);
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/findings/f3/ignore", { method: "POST" }), {
      params: { id: "f3" },
    });
    expect(res.status).toBe(409);
    expect(rows.find((r) => r.id === "f3")?.status).toBe("IGNORED");
  });

  it("returns 409 for an APPROVED finding (Approved is terminal in this module's scope)", async () => {
    resetFakeDb([makeRow({ id: "f4", status: "APPROVED" })]);
    const { POST } = await import("./route");
    const res = await POST(new Request("http://localhost/api/findings/f4/ignore", { method: "POST" }), {
      params: { id: "f4" },
    });
    expect(res.status).toBe(409);
  });
